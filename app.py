                        ...
                        try:
                            ...
                            while i < len(lines):
                                ...
                                if 'R$' in line and any(c.isdigit() for c in line):
                                    ...
                                    # Sobe do preço até a trava de segurança do item anterior
                                    for j in range(i-1, max(-1, ultimo_i_nome), -1):
                                        txt = lines[j].strip()
                                        txt_low = txt.lower()
                                        
                                        # NOVA REGRA: Filtro de quantidade mínima de palavras (>= 3)
                                        if any(c.isalpha() for c in txt) and 'R$' not in txt and len(txt.split()) >= 3:
...
# EXIBIÇÃO ESTÁVEL COM PANDAS E ARROW
